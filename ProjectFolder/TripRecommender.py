...
# 필요한경우 띄어쓰기 제거 (e.g. 인근 지역 -> 인근지역)
    ...
    string = string.replace('전문 박물관', '전문박물관')
    string = string.replace('어린이 박물관', '어린이박물관')
    string = string.replace('자연사 박물관', '자연사박물관')
    string = string.replace('군사 박물관', '군사박물관')
    ...
    string = string.replace('물건 찾기 게임', '물건찾기게임')
    string = string.replace('테마 파크', '테마파크')
    string = string.replace('대중교통 시스템', '대중교통시스템')
    string = string.replace('스포츠 컴플렉스', '스포츠컴플렉스')
    string = string.replace('부두 보드워크', '부두보드워크')
...
def duplicates_handle(string):
    each_words = list(set(string.split(' ')))
    
    result = each_words.copy()
    for i in range(len(each_words)):
        if each_words.count(each_words[i])!=1:
            result.remove(each_words[i])
